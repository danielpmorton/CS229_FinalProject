...
def getTrendsDataNew(startDate, endDate, geo, queries):
    # INPUTS ------------------------
    # startDate  | example: '2020-01-22' -- of form YEAR-MO-DY
    # endDate    | example: '2020-04-22'
    # geo        | example: 'US-CA'
...
def getAllJHUdataNew(state, JHU_startDate, JHU_endDate):
...
def getDataNew(startDateX, endDateX, startDateY, endDateY, geo, state):
    ...
    # Querying Google trends API
    trends_X = getTrendsDataNew(startDateX, endDateX, geo, queries)
    # Put into a dataframe with labeled columns with the search queries
    trends_X_df = pd.DataFrame(trends_X, columns=queries)
