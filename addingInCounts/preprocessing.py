...
def getData(startDateX, endDateX, startDateY, endDateY, geo, state):
    ...
    d1_minus1 = d1 - timedelta(days=1)
    d2 = datetime.strptime(endDateY, '%Y-%m-%d')
    d2_minus1 = d2 - timedelta(days=1)
    num_days = (d2-d1).days +1 # Adding 1 to match Megan's values (inclusivity of the end date?)
    ...
    if platform.system() == 'Windows':
        ...
        d1minus1_JHU = d1_minus1.strftime('%#m/%#d/%y')
        d2minus1_JHU = d2_minus1.strftime('%#m/%#d/%y')
    else:
        startDate_JHU_format = d1.strftime('%-m/%-d/%y') # Converting from date format for trends to JHU df labels
    ...
    # Adding a column to the X dataframe containing the case count from the previous day
    cases_prevDay = dailyCases[d1minus1_JHU:d2minus1_JHU]
    trends_X_df['Cases From Previous Day'] = cases_prevDay
